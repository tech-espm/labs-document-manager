...
$.fn.dataTable.ext.type.order["customdate-pre"] = function (d) {
	if (!d)
		return -2147483648;
...
window.prepareDataTable = function (id, opt) {
	...
	if (removeDom === "f") {
		...
		prepareCustomFilter(dt, id, "", "Filtro");
		if (removeDom === "l") {
			f = _(id + "_length");
			if (f) {
		...
		if (o.export) {
			addFilterButton(id + "_filter", "fa-nomargin fa-file-text", null, function () {
			...
			}).setAttribute("title", "Exportar CSV");
		}
	}
...
window.DataTableExporter = {
	alertNothingToExport: function () {
		Notification.error("Não há dados disponíveis para exportação \uD83D\uDE22", true);
		return false;
	},
	...
	normalizeConfig: function (config, extension) {
		...
		config.title = (config.title || "Dados").replace(config.regex1, "").replace(config.regex2, " ").replace(config.regex3, "");
	...
	exportCSV: function (dt, config) {
		...
		var i, c, blobdata, b = 0, colhdr = [], colidx = [], colDataIdx = [], th, colcount = dt.columns().indexes().length, rownodes, rowcount = dt.rows().count(), rowarray, filterCount = (!config.filters ? 0 : config.filters.length), colsettings = dt.settings()[0].aoColumns, tmpDiv = document.createElement("div"),
		...
		for (i = 0; i < colcount; i++) {
			...
			colhdr.push((((th = colsettings[i].nTh) && th.getAttribute("title")) || colsettings[i].sTitle).toString().replace(config.regex1, "").replace(config.regex2, " ").replace(config.regex3, "").replace(config.regexSemicolon, ","));
			colidx.push(i);
			colDataIdx.push(colsettings[i].mData);
		}
		...
		for (i = 0; i < rowcount; i++) {
			...
			for (c = 0; c < colcount; c++) {
				...
				if (colsettings[colidx[c]].mRender) {
					...
					blobdata[b++] = rowarray[colDataIdx[c]].toString().replace(config.regex2, " ").replace(config.regex3, "").replace(config.regexSemicolon, ",");
				} else {
					// Otherwise, remove the tags and replace the rest
					tmpDiv.innerHTML = rowarray[colDataIdx[c]].toString().replace(config.regex1, "").replace(config.regex2, " ").replace(config.regex3, "");
