...
window.prepareDataTable = function (id, opt) {
	...
	if (id === "dataTableMain") {
		if (window.location.pathname)
			f = window.location.pathname.toLowerCase();
...
window.DataTableExporter = {
	...
	exportCSV: function (dt, config) {
		...
		var i, c, blobdata, b = 0, colhdr = [], colidx = [], colDataSrc, colDataIdx = [], colDataMustUseEval = [], th, colcount = dt.columns().indexes().length, rownodes, rowcount = dt.rows().count(), rowarray, filterCount = (!config.filters ? 0 : config.filters.length), colsettings = dt.settings()[0].aoColumns, tmpDiv = document.createElement("div"),
		...
		for (i = 0; i < colcount; i++) {
			...
			colidx.push(i);
			colDataIdx.push(colDataSrc = colsettings[i].mData);
		...
		for (i = 0; i < rowcount; i++) {
			...
			for (c = 0; c < colcount; c++) {
				...
				if (colsettings[colidx[c]].mRender) {
					...
					blobdata[b++] = (colDataMustUseEval[c] ? eval("rowarray." + colDataIdx[c]) : rowarray[colDataIdx[c]]).toString().replace(config.regex2, " ").replace(config.regex3, "").replace(config.regexSemicolon, ",");
				} else {
					// Otherwise, remove the tags and replace the rest
					tmpDiv.innerHTML = (colDataMustUseEval[c] ? eval("rowarray." + colDataIdx[c]) : rowarray[colDataIdx[c]]).toString().replace(config.regex1, "").replace(config.regex2, " ").replace(config.regex3, "");
